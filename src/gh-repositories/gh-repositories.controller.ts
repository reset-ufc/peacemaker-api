...
export class GhRepositoriesController {
  ...
  @Get('remote')
  async getRemoteRepositories(@Res() response: Response, @CurrentUser() user: User) {
    const repositories = await this.ghRepositoriesService.findRemoteRepositories(
  ...
  @Get('local')
  async getLocalRepositories(@Res() response: Response, @CurrentUser() user: User) {
